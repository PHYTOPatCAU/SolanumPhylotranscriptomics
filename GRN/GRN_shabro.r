...
modules <- read.delim("/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/WGCNA/shabro/TOM/TOMfinal_module_colors_geneids.txt", row.names=1,sep=" ")
GO_TERM <- read.csv("/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/GO_terms/shabr/filtered_protein_names_shabro.csv", header=T, row.names=1)
wgcna_hub <- read.csv("/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/WGCNA/shabro/HUB/2025_01_10_shabro_hub.csv", row.names=1)
...
TF <- read.delim("GRN/data/shabro_TFs/shabro_TFs.txt", header=F)
...
breakpont <- brkpnt_fun(wam_linked_list$weight,7,paste0("GRN/documentation/pics/", Sys.Date(), "_shabro_breakpoints.png"))

# picked 0.00392887824766625
# filter dataframe with eigencentrality threshold
grn_edges_all <- wam_linked_list[wam_linked_list$weight > 0.00392887824766625,]
...
breakpont <- brkpnt_fun((grn_all_eigen$egnvctr),5, paste0("GRN/documentation/pics/", Sys.Date(), "_shabro_breakpoints_hubs.png"))
# selected 0.132488469786843
hubs <- grn_all_eigen[grn_all_eigen$egnvctr > 0.132488469786843,]
...
write_delim(genes_names_wth_hubs, file = "/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/GRN/shabro_grn_network_nodes.txt", delim = "\t")

# output GRN network
write_delim(grn_edges_all, file = "/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/GRN/shabro_grn_network_edges.txt", delim = "\t")
