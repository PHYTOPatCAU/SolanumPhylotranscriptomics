...
# Perform GRN analysis on  S. chilense
# Severin Einspanier
# 2025-01-10
...
ids <- read.delim("schil_curated_proteome_OG_pannzer_dedub_ids.txt", header=F) %>%
    mutate(gene=gsub(">", "", V1)) %>%
...
modules <- read.delim("moduleColorsTOM_ds2_mch25_genids.txt", row.names=1, sep=" ")
GO_TERM <- read.csv("filtered_protein_names_schil.csv", header=T, row.names=1)
wgcna_hub <- read.csv("2025_01_10_schil_hub.csv", row.names=1)
...
TF <- read.delim("schil_TFs.txt", header=F)
...
breakpont <- brkpnt_fun(wam_linked_list$weight, 7, paste0("schil_breakpoints.png"))
...
breakpont <- brkpnt_fun((grn_all_eigen$egnvctr), 5, paste0("schil_breakpoints_hubs.png"))
...
write_delim(genes_names_wth_hubs, file = "schil_grn_network_nodes.txt", delim = "\t")

# output GRN network
write_delim(grn_edges_all, file = "schil_grn_network_edges.txt", delim = "\t")