...
# Genes of interest: Upregulated in the resistant S. pennellii genotype
spen_genes <- c("Sopen09g001470", "Sopen10g006210", "Sopen05g003630")

# Which Orthogroups do these genes belong to?
OG_of_interest <- spenID2OG %>%
  filter(GeneID %in% spen_genes)

# Confirm these OGs are found in all species (optional check)
OG_info <- read_tsv(
...
#==============================
OGs <- c("OG0005445","OG0007365","OG0015518")
...
schil_module <- read.table("schil/moduleColorsTOM_ds2_mch25_genids.txt",
                           header = TRUE) %>%
...
make_anno_and_mat <- function(og_id) {
  ...
  ann_df <- merged %>%
    filter(OG == og_id) %>%
    left_join(all_modules, by = c("species", "GeneID")) %>%
...
og1_data <- make_anno_and_mat("OG0005445")
og2_data <- make_anno_and_mat("OG0015518")
og3_data <- make_anno_and_mat("OG0007365")
...
col_fun1 <- circlize::colorRamp2(c(0, 5),   c("white", "darkred"))
col_fun2 <- circlize::colorRamp2(c(0, 3),   c("white", "darkorange"))
col_fun3 <- circlize::colorRamp2(c(0, 10),  c("white", "darkred"))
...
lgd1 <- Legend(title = "log2FC \nOG0005445", col_fun = col_fun1)
lgd2 <- Legend(title = "log2FC \nOG0015518", col_fun = col_fun2)
lgd3 <- Legend(title = "log2FC \nOG0007365", col_fun = col_fun3)
...
png(
  filename = paste0(
    "figures/fig_7/",
    Sys.Date(),
    "_spen_resis_TFs_expression_across.png"
  ),
  width  = 16.5, 
  height = 19.5, 
  units  = "cm", 
