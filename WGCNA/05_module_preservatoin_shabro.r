...
datExpr <- read.csv("DeSeq/data/norm_counts_all_rlog.csv") %>% 
  dplyr::filter(species=="S. habrochaites") %>% 
...
modules <- read.table("/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/WGCNA/shabr/TOM/moduleColorsTOM_ds1_mch02_genids.txt", 
    sep=" ", row.names=NULL) %>%
...
write.table(zstat, "/gxfs_work/cau/suaph281/RNAseq/RNAseq_work/data/WGCNA/shabr/zstat/2024_12_01_zstat.tsv", sep="\t", row.names = FALSE)

png("WGCNA/documentation/pics/shabro/presevation_zstats.png", width = 800, height = 400)
...
png("WGCNA/documentation/pics/shabro/presevation_median_rank.png", width = 800, height = 400)
