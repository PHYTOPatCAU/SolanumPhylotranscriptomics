...
datExpr <- read.csv("DeSeq/data/norm_counts_all_rlog.csv") %>%
  filter(species=="S. habrochaites" & Geneid %in% ids$gene) %>%
...
# Plot heatmap
png(paste0("WGCNA/documentation/pics/shabro/", Sys.Date(),
           "_heatmap_module_trait.png"),
    width = 18/2, height = 10, units = "cm", res = 900)
svg(paste0("WGCNA/documentation/pics/shabro/", Sys.Date(),
