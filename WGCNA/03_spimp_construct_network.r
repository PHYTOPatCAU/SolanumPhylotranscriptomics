...
png(file = "WGCNA/documentation/pub/spimp_network_filtered_sft9.png", width = 2000, height = 2000, 
    res=600);
...
svg(file = "WGCNA/documentation/pub/spimp_network_filtered_sft9.svg", width = 7, height = 5)
plotDendroAndColors(result$dendrograms[[1]], 
                      colors[result$blockGenes[[1]]], 
                      c(""), 
                      dendroLabels = FALSE, hang = 0.03,
...
combined_results$hub <- ifelse(combined_results$module == "pink" & as.numeric(combined_results$position) >= 121, "hub", combined_results$hub)
