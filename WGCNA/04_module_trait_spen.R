...
datExpr <- read.csv("DeSeq/data/norm_counts_all_rlog.csv") %>%
  filter(species=="S. pennellii" & Geneid %in% ids$gene) %>%
...
moduleColors <- read.table("WGCNA/data/colors/spen_module_colors_TOM_filtered_genids.txt", header=TRUE)
...
# Plot heatmap
png(paste0("WGCNA/documentation/pics/spen/", Sys.Date(),
           "_heatmap_module_trait_eta2_interaction.png"),
    width = 16.5/2, height = 8, units = "cm", res = 900)
svg(paste0("WGCNA/documentation/pics/spen/", Sys.Date(),
